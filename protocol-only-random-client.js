...
const DEFAULT_PROXY_URL = 'datacenter-ww.lightningproxies.net:1339:demo_user-ttl-0:demo_pass_xxxxx';
...
const MOVEMENT_INTERVAL_MS = Number(process.env.ARRAS_MOVEMENT_INTERVAL_MS || 90);
const MOVEMENT_JITTER_MS = Number(process.env.ARRAS_MOVEMENT_JITTER_MS || 25);
const MOVEMENT_PHASE_JITTER_MS = Number(process.env.ARRAS_MOVEMENT_PHASE_JITTER_MS || 100);
...
const FIRE_STAGGER_MS = Number(process.env.ARRAS_FIRE_STAGGER_MS || 280);
const FIRE_PULSE_ON_MS = Number(process.env.ARRAS_FIRE_PULSE_ON_MS || 50);
const FIRE_PULSE_OFF_MS = Number(process.env.ARRAS_FIRE_PULSE_OFF_MS || 450);
...
const MANUAL_CONTROL_INTERVAL_MS = Number(process.env.ARRAS_MANUAL_CONTROL_INTERVAL_MS || 110);
...
const CLASS_UPGRADE_INITIAL_DELAY_MS = Number(process.env.ARRAS_CLASS_UPGRADE_INITIAL_DELAY_MS || 1200);
const CLASS_UPGRADE_STEP_DELAY_MS = Number(process.env.ARRAS_CLASS_UPGRADE_STEP_DELAY_MS || 250);
const CLASS_UPGRADE_STAT_DELAY_MS = Number(process.env.ARRAS_CLASS_UPGRADE_STAT_DELAY_MS || 900);
const SLOW_MODE_CLASS_UPGRADE_STEP_DELAY_MS = Number(process.env.ARRAS_SLOW_MODE_CLASS_UPGRADE_STEP_DELAY_MS || 850);
const SLOW_MODE_CLASS_UPGRADE_STAT_DELAY_MS = Number(process.env.ARRAS_SLOW_MODE_CLASS_UPGRADE_STAT_DELAY_MS || 1500);
const SLOW_CLASS_UPGRADE_HASHES = new Set(
  String(process.env.ARRAS_SLOW_CLASS_UPGRADE_HASHES || 'wb')
    .split(',')
...
const AUTO_RESPAWN_DELAY_MS = Number(process.env.ARRAS_PROTOCOL_AUTO_RESPAWN_DELAY_MS || 1200);
...
async function buildProxyAgent() {
  const proxyUrl = normalizeProxyUrl(process.env.ARRAS_PROXY_URL || firstProxyFromFile() || DEFAULT_PROXY_URL);
  if (!proxyUrl) {
    return null;
  }
  try {
...
class ProtocolOnlyRandomClient {
  ...
  maybeSpawn(reason) {
    ...
    const hasName = Boolean(this.defaultSpawnName);
    const hasParty = Boolean(this.options.party);
    const usedTemplateSpawn = Boolean(this.spawnTemplate && !hasName && !hasParty);
    const packet = usedTemplateSpawn ? this.spawnTemplate : buildDefaultSpawnPacket(this.defaultSpawnName, this.options.party || '');
    ...
    if (!usedTemplateSpawn && !hasName && !this.options.party) {
      ...
      console.log(`[OUT] spawn custom-shape bytes=${packet.length} name=${JSON.stringify(this.defaultSpawnName)} party="${this.options.party || ''}" via=${reason}`);
    }
  }
  ...
  scheduleClassUpgrade(reason) {
    ...
    this.upgradeStarted = true;
    const slowMode = this.usesSlowClassUpgradeTiming();
    const stepDelay = slowMode ? SLOW_MODE_CLASS_UPGRADE_STEP_DELAY_MS : CLASS_UPGRADE_STEP_DELAY_MS;
    const statDelay = slowMode ? SLOW_MODE_CLASS_UPGRADE_STAT_DELAY_MS : CLASS_UPGRADE_STAT_DELAY_MS;
    this.log(
      `[OUT] upgrade schedule tank=${this.targetTank} steps=${pathSteps.map(formatUpgradeStep).join('')} ` +
      `delay=${stepDelay}ms via=${reason}`
    );
    let offset = CLASS_UPGRADE_INITIAL_DELAY_MS;
  ...
  usesSlowClassUpgradeTiming() {
    const modeId = this.modeInfo && typeof this.modeInfo.id === 'string'
      ? this.modeInfo.id.replace(/^#/, '').toLowerCase()
      : '';
    const urlHash = this.hashFromUrl().replace(/^#/, '').toLowerCase();
    return SLOW_CLASS_UPGRADE_HASHES.has(modeId) || SLOW_CLASS_UPGRADE_HASHES.has(urlHash);
  }
  ...
  resolveMouseDown(rawMouseDown, now) {
    ...
    const elapsed = now - this.fireHoldStartedAt;
    if (elapsed < this.fireStartDelayMs) {
      return false;
    }
    ...
    const pulsePeriod = Math.max(1, FIRE_PULSE_ON_MS + FIRE_PULSE_OFF_MS);
    return ((elapsed - this.fireStartDelayMs) % pulsePeriod) < FIRE_PULSE_ON_MS;
  }
  ...
  updateControlState(message) {
    ...
    const manualAimWithMovement = manualMode && (Boolean(message.manualAimWithMovement) || MANUAL_AIM_WITH_MOVEMENT);
    ...
    this.controlState = {
      ...
      manualMode,
      ignoreControllerAim: manualAimWithMovement,
